...
//NOTE: Add specific debug information
#define KEYWORD "[LAB1112]"
#define KERN_LOG KERN_WARNING KEYWORD
...
static int parse_txq_params_sp(uint32_t data,
				struct ieee80211_txq_params *txq_params)
{
	txq_params->ac = (data & TXQ_MASK_AC)>>0;
	txq_params->aifs = (data & TXQ_MASK_AIFS)>>2;
	txq_params->cwmin = (data & TXQ_MASK_CWMIN)>>6;
	txq_params->cwmax = (data & TXQ_MASK_CWMAX)>>19;
	txq_params->txop = 0;

	/* printk(KERN_LOG "AC-%d, AIFS-%d, [%d, %d], TXOP-%d.\n", \
					txq_params->ac, txq_params->aifs, \
					txq_params->cwmin, txq_params->cwmax, \
					txq_params->txop); */
	return 0;
}
			    ...
			    struct ieee80211_txq_params *txq_params)
{
	...
	{
		printk(KERN_LOG "TXQ ATTR: AC, %p.\n", tb[NL80211_TXQ_ATTR_AC]);
		printk(KERN_LOG "TXQ ATTR: TXOP, %p.\n", tb[NL80211_TXQ_ATTR_TXOP]);
		printk(KERN_LOG "TXQ ATTR: CWMIN, %p.\n", tb[NL80211_TXQ_ATTR_CWMIN]);
		printk(KERN_LOG "TXQ ATTR: CWMAX, %p.\n", tb[NL80211_TXQ_ATTR_CWMAX]);
		printk(KERN_LOG "TXQ ATTR: AIFS, %p.\n", tb[NL80211_TXQ_ATTR_AIFS]);
		return -EINVAL;
	}
	else
	{
		printk(KERN_LOG "TXQ ATTR: everything is cool.\n");
	}
	...
	{
		printk(KERN_LOG "TXQ PARSE: %d exceeded max_acs_num.\n", txq_params->ac);
		return -EINVAL;
	}
...
//NOTE: nl80211_set_wiphy, nla_nest_params
static int nl80211_set_wiphy(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (info->attrs[NL80211_ATTR_IFINDEX]) {
		...
		// printk(KERN_LOG "WIPHY_SET: if-%d\n", ifindex);
	...
	if (!netdev) {
		printk(KERN_LOG "WIPHY_SET: NO netdev being set.\n");
	...
	if (info->attrs[NL80211_ATTR_WIPHY_TXQ_PARAMS]) {
		...
		// printk(KERN_LOG "AT LEAST, TXQ_PARAMS here.\n");
		...
		//NOTE: modified; not nested strcuture any more;
		data = nla_get_u32(info->attrs[NL80211_ATTR_WIPHY_TXQ_PARAMS]);
		result = parse_txq_params_sp(data, &txq_params);
		//result is always TRUE here

		result = rdev_set_txq_params(rdev, netdev,
									&txq_params);
		if (result)
		{
			printk(KERN_LOG "TXQ_PARAMS: set failed.\n");
			return result;
		}
		/* nla_for_each_nested(nl_txq_params,
				    info->attrs[NL80211_ATTR_WIPHY_TXQ_PARAMS],
				    rem_txq_params) {
			printk(KERN_LOG "TXQ_PARAMS: remains %d before parsing.\n", \
				rem_txq_params - NLA_ALIGN(nl_txq_params->nla_len));
			...
			if (result)
			{
				printk(KERN_LOG "TXQ_PARAMS: getAttrs failed.\n");
				return result;
			}
			...
			if (result)
			{
				printk(KERN_LOG "TXQ_PARAMS: getValues failed.\n");
				return result;
			}
...
static const struct genl_ops nl80211_ops[] = {
	...
	{
		.cmd = NL80211_CMD_SET_WIPHY, //NOTE: nl80211_set_wiphy
